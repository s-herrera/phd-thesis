...
if __name__ == "__main__":
    ...
    data, languages = extract_data(args.data, scope, conclusion, conclusion_meta, feature_predicate, include_metadata)
    ...
    extracted_rules['X'] = X
    extracted_rules['y'] = y
    extracted_rules['feature_names'] = [name for f in feature_set.features for name in f.get_all_names()]
    extracted_rules['languages'] = tuple(languages)
    extracted_rules['data'] = data
        ...
        for name, (value, idx) in feature_set.feature_weights(model.coef_[0]).items():
            if name not in all_rules:
                ...
                ordered_rules.append({
                    ...
                    "matches": tuple(with_feature_selector)
                })
...
with open(args.output.split(".")[0] + ".pickle", 'wb') as out_stream:
    pickle.dump(extracted_rules, out_stream)

del extracted_rules['X']
del extracted_rules['y']
del extracted_rules['languages']
del extracted_rules['data']
del extracted_rules['feature_names']
